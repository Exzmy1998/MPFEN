"""MPFEN 推理入口：支持分块预测、视频分割、FPS 测试和 ONNX 导出。

单图模式输出二值裂缝掩码，目录模式保存彩色分割结果。
"""
...
if __name__ == "__main__":
    ...
    # 模式：predict（单图二值掩码）、video、fps、dir_predict（彩色图）、export_onnx。
    mode = "predict"
    ...
    if mode == "predict":
        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(
                ...
                r_image = mpfen.detect_image(image)
