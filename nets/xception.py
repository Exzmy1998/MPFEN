...
class Xception(nn.Module):
    """输出浅层与深层特征的 Xception 骨干，支持 8 或 16 倍下采样。"""

    def __init__(self, downsample_factor):
        """根据 downsample_factor 配置输出步幅和空洞卷积倍率。"""
...
def xception(pretrained=True, downsample_factor=16):
    """创建 Xception 骨干，并按需加载 ImageNet 预训练权重。"""
    model = Xception(downsample_factor=downsample_factor)
    if pretrained:
        # 使用上游提供的骨干权重，保留其真实下载地址。
